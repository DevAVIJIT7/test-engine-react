...
class TestCreate extends React.Component {
  ...
  renderStudentList = () => {
    ...
    return (
      <div className={`dropdown-menu ${showDropdownList ? 'show' : ''}`}>
        <div className="items">
          {this.filteredStudents().map((s) => (
            <div className="dropdown-item" key={s.id}>
              <input type="checkbox" value={s.id} onChange={this.selectStudent} checked={s.checked} />
              <span>{s.name}</span>
            </div>
          ))}
        </div>
  ...
  filteredStudents = () => {
    const { students, searchName } = this.state;
    const searchRegx = new RegExp(searchName, 'i');
    return students.filter((s) => searchRegx.test(s.name));
  }
    ...
    return (
      <div>
        ...
        <div className="test-create-box shadow row">
            ...
            <form>
              <div className="form-row">
                <div className="col-md-4 mb-3">
                  <Field name="title" label="Title" value={testTitle} component={this.renderInput} onChange={this.changeTitle} />
                </div>
                <div className="col-md-4 mb-3">
          ...
          <div className="col-lg-1 vertical-line" />
